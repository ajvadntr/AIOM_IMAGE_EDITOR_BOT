class script(object):

    START_MSG = """ <b>Hello {}

I'm a Image Editor Bot Which Supports Various Modes

For More Click /help....</b>"""
    HELP_MSG = """Hey, Follow These Steps...

<code>⭕️ Send Me Any Image To Edit..</code>

<code>⭕️ Select The Corresponding Mode That You Need</code>

<code>⭕️ Your Edited Image Will Be Uploaded </code>


- @AIOM_BOTS -"""
    ABOUT_MSG = """<b>My Name :</b> <a herf='https://t.me/AIOM_IMAGE_EDITOR_BOT'>Image Editor Bot</a>

<b>Language :</b> <code>Python3</code>

<b>Library :</b> <a href='https://docs.pyrogram.org/'>Pyrogram 1.0.7</a>

<b>Source Code :</b> <a href='https://github.com/ajvadntr/AIOM_IMAGE_EDITOR_BOT'>Click Here</a>"""
